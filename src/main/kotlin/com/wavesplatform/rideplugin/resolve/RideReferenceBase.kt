...
abstract class RideReferenceBase(@JvmField protected val psiElement: PsiElement, range: TextRange) :
    ...
    override fun multiResolve(incompleteCode: Boolean): Array<ResolveResult> {
        return ResolveCache.getInstance(psiElement.project).resolveWithCaching(
            this, { rideReferenceBase, _ ->
                rideReferenceBase.resolveInner(false)
...
//        if (!RideCompletionUtil.shouldComplete(psiElement)) {
//            return emptyArray()
//        }
    ...
    override fun equals(other: Any?): Boolean {
        return if (other is RideReferenceBase) psiElement == other.psiElement else false
    }
    ...
    protected val namedElementOwner: PsiElement?
        get() {
            val namedElement = psiElement.parentOfType<RideNamedElement>()
